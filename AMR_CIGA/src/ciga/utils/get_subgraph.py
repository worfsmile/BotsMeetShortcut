...
def relabel_nodes(x, edge_index, sub_nodes, batch, pos=None):
    num_nodes = x.size(0)
...
#     full_graph_ids = torch.unique(batch)
#     kept_graph_ids = torch.unique(batch[sub_nodes])
#     missing_graph_ids = torch.tensor(
#         [g.item() for g in full_graph_ids if g not in kept_graph_ids],
#         device=x.device, dtype=torch.long
#     )

#     if len(missing_graph_ids) > 0:
#         mask = torch.isin(batch, missing_graph_ids)
#         extra_nodes = torch.nonzero(mask, as_tuple=False).view(-1)
#         sub_nodes = torch.cat([sub_nodes, extra_nodes])
    ...
    for edge_index, N, C, E in zip(edge_indices, num_nodes.tolist(), cum_nodes.tolist(), num_edges.tolist()):
        if connectivity:
            ...
            for num_hops in range(1, MAX_DIAM):
                _, _, _, broken_mask = bid_k_hop_subgraph(node_idx=node_idx,
                                                          num_hops=num_hops,
