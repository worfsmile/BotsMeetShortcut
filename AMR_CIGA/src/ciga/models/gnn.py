...
class GNN(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if pred_head == "cls":
            if graph_pooling == "set2set":
                self.graph_pred_linear = torch.nn.Linear(2 * (self.emb_dim + self.add_info), self.num_class)
            else:
                self.graph_pred_linear = torch.nn.Linear(self.emb_dim+self.add_info, self.num_class)
        elif pred_head == "inv":
            self.graph_pred_linear = torch.nn.Sequential(nn.Linear(emb_dim+add_info, 2 * (emb_dim)), nn.ReLU(),
                                                         nn.Linear(2 * (emb_dim), self.num_class))

            self.spu_mlp = torch.nn.Sequential(nn.Linear((emb_dim+add_info), 2 * emb_dim), nn.ReLU(),
                                                nn.Linear(2 * emb_dim, self.num_class))
    ...
    def forward(self, batched_data, get_rep=False):
        ...
        h_graph = self.pool(h_node, batched_data.batch)
        if self.add_info:
            h_graph = torch.cat([h_graph, batched_data.cat_tensor], dim=-1)
        if get_rep:
