...
def run_one_dataset(dataset, text, feature, train_type, device, global_table, ece_table):
    train_loader, test_loader,val_loader, input_size = get_dataloader(dataset, text, feature, train_type, mod_tag, mod_flag)
    model = MLP([input_size]).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    criterion = nn.CrossEntropyLoss()
    logname = f"{dataset}_{text}_{feature}_{train_type}"
    model, best_acc, ece = run((model, optimizer, criterion), (train_loader, test_loader, val_loader), device, logname)
...
# train_settings = ['llm_enhance1', 'llm_enhance2', 'llm_enhance3', 'ood2']
        ...
        for text in texts:
            ...
            for feature in features:
                ...
                for train_type in train_types:
                    ...
                    global_table, ece_table = run_one_dataset(dataset, text, feature, train_type, "cuda:7", global_table, ece_table)
